...
declare module "http" {
  interface IncomingMessage {
    purviewState?: {
      wsState: Purview.WebSocketState
      idStateTrees: Purview.IDStateTree[]
      roots?: Purview.ConnectedRoot[]
    }
    purviewCSSState?: Purview.CSSState
...
namespace Purview {
  ...
  export type WebSocketState = WebSocketStateHasCSS | WebSocketStateNoCSS
  ...
  export function createElem(
    ...
    if (isCheckedInput) {
      console.log("calling createElem, isCheckedInput", isCheckedInput)
    }
      ...
      if (root.connected) {
        ...
        switch (eventName) {
          ...
          case "change":
            ...
            if (nodeName === "input") {
              const type = (attributes as JSX.InputHTMLAttributes)
                .type as string
...
export type ComponentConstructor<P, S> = Purview.ComponentConstructor<P, S>
export type Component<P, S> = Purview.Component<P, S>
export type Fragment<P extends { children: any }> = Purview.Fragment<P>
export type CSSState = Purview.CSSState
export type StateTree = Purview.StateTree
