...
export function toHTML(pNode: PNodeRegular): string {
  const sel = pNode.sel
...
function escapeTextInRawElement(str: string): string {
...
const validatedTagCache: Record<string, boolean> = {}
function validateDangerousTag(tag: string): boolean {
...
function escapeHtml(origStr: string): string {
  const str = "" + origStr
 ...
 */
function escapeTextForBrowser(text: string | boolean | number): string {
